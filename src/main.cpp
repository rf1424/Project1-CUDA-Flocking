...
void initShaders(GLuint * program) {
  ...
  void mainLoop() {
    ...
    // fps measurement 
    const double WARMUP_SEC = 2.0;
    const double MEASURE_SEC = 5.0;
    double runStart = glfwGetTime();
    bool warmedUp = false;
    ...
    while (!glfwWindowShouldClose(window)) {
      ...
      if (!warmedUp && time - runStart > WARMUP_SEC) {
        warmedUp = true;
        measureStart = time;
      }

      if (warmedUp && !reported) {
        measureFrames++;
        if (time - measureStart > MEASURE_SEC) {
          double avgFps = measureFrames / (time - measureStart);
          printf("=== AVG FPS over %.1fs (after %.1fs warmup): %.2f ===\n", MEASURE_SEC, WARMUP_SEC, avgFps);
          reported = true;
        }
      }
